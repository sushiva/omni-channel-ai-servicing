...
from src.services.email_config import EmailConfig
from src.integrations.email_client import EmailClient, EmailMessage
...
class EmailPoller:
    ...
    async def _send_email_response(
        ...
        """
        # For now, just log
        # In production, this would use SMTP or notification service
        logger.info(f"📧 Would send email response to {to_email}: {subject}")
        logger.debug(f"Response body: {body[:200]}")
